...
const FormStepTwo = () => {
    ...
    useEffect(() => {
        try {
            axiosClient.get("/api/cities").then((res) => {
                setCities(res.data)
            })
        } catch (err) {
            console.log(err)
    ...
    return (
        <div className="bg-white shadow-sm border border-slate-100 p-6 md:p-8 rounded-[2rem]">
                ...
                <div className="flex flex-col gap-2 w-full">
                    ...
                    <div className="relative w-full">
                        ...
                        >
                            <option value="">Sélectionner une ville</option>
                            {cities
                                .filter((city) => city.id !== Number(cityOne))
                                .map((city) => (
                                    <option
                                        className="capitalize"
                                        key={city.id}
            ...
            <div className="mt-8 flex flex-col gap-2 w-full">
                <label className="text-sm text-slate-700 font-bold">
                    Quand doit-il être livré au plus tard ?
                </label>
                <input
