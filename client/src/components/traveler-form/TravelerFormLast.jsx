...
const TravelerFormLast = () => {
    ...
    const selectedCityTwo = cities.find(
        (city) => city.id === Number(travelCityTwo),
    )
    ...
    const tripSummary = {
        departure: {
            ...
            city: selectedCityOne?.name || "",
        },
        arrival: { label: "LIEU D'ARRIVÉE", city: selectedCityTwo.name || "" },
        details: [
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white shadow-sm border border-slate-100 p-6 rounded-[2rem]">
                <h2 className="text-lg font-bold text-[#0984E3]">
                    Résumé du trajet
                </h2>
                ...
                <div className="flex flex-col gap-1 mb-6">
                    <div className="flex items-center gap-3">
                        <span className="w-8 h-8 rounded-full bg-[#0984E3] text-white flex items-center justify-center text-xs font-bold shrink-0">
                            <MapPin size={14} />
                        </span>
                        <div>
                            <p className="text-[10px] font-bold text-slate-400 tracking-widest">
                                {tripSummary.departure.label}
                            </p>
                            <p className="font-bold text-slate-800 capitalize">
                                {tripSummary.departure.city}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <span className="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center text-xs font-bold shrink-0">
                            <MapPin size={14} />
                        </span>
                        <div>
                            <p className="text-[10px] font-bold text-slate-400 tracking-widest">
                                {tripSummary.arrival.label}
                            </p>
                            <p className="font-bold text-slate-800 capitalize">
                                {tripSummary.arrival.city}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    {tripSummary.details.map((item, i) => (
                        ...
                        >
                            <div>
                                ...
                                <p className="text-sm font-semibold text-[#0984E3] capitalize">
                                    {item.value}
                                </p>
                            </div>
                        </div>
